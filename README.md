...
Current runtime behavior is still baseline:
- Movement is policy-routed with heuristic fallback.
...
- Full provider-backed cognition execution is still pending (heuristic route remains active).
...
LLM provider keys:
- Default: platform-managed (`OPENAI_API_KEY` on server/operator side).
- Optional BYOK mode: set `VVALLEY_REQUIRE_USER_LLM_API_KEY=true`.
